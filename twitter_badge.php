<?php
	...
	*/
	...
	class Twitter_Badge {
		...
		var $url = 'http://twitter.com/statuses/user_timeline.rss?screen_name=';
		...
		public function display_tweets()
		{
			...
			foreach ($this->tweets as $update) {
				$item  = "<li>{$update['status']}&nbsp;<a href=\"{$update['link']}\">";
		...
		private function load_tweets()
		{
			...
			// count is checked in case there was an error
		...
		private function fetch_tweets()
		{
			...
			@$doc->load($this->url.$this->username); // fetch the user's RSS feed
			...
			foreach ($doc->getElementsByTagName('item') as $node) {
				...
				// url encode quotes just to be safe; don't want this to break our html in any way
		...
		private function load_file_cache()
		{
			...
			if (!$data) // the cache file might not exist yet
			{
				return;
			}
